...
class PoolManager(object):
    ...
    def process_message(self, message):
        if isinstance(message, Acknowledgement):
            ...
            self.process_results(message)

    def process_acknowledgement(self, ack):
        try:
            self.worker_manager.workers.get(ack.worker)
            self.task_manager.task_start(ack.task, ack.worker)
        except KeyError:  # ACK from stopped worker
            task = self.task_manager.tasks[ack.task]
            self.worker_manager.dispatch(task)

    def process_results(self, results):
        try:
            self.task_manager.task_done(results.task, results.results)
        except KeyError:  # delivered results of timeout/cancelled task
            pass

    def update_status(self):
        timestamp = time.time()

        if timestamp - self.last_status_update >= SLEEP_UNIT:
            self.inspect_tasks()
    ...
    def inspect_workers(self):
        for worker_id, exitcode in self.worker_manager.inspect_workers():
            ...
            except LookupError:  # termination due to task timeout/cancellation
                pass
            else:
    ...
    def worker_id_lookup(self, worker_id):
        for task in self.task_manager.tasks.values():
            ...
            raise LookupError("Worker has not associated task")
